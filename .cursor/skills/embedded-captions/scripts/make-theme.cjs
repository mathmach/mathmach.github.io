...
function probeDuration() {
  try {
    const out = execFileSync(
      'ffprobe',
      ['-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0', path.join(PROJECT, 'source.mp4')],
      { encoding: 'utf8' },
    );
...
function sceneHeroXY(setpiece, fontPx) {
  ...
  let x = exX ?? null,
    y = exY ?? null;
  if (x == null && sz && sz.heroAnchor && sz.heroAnchor.centerXPct != null) x = (W * sz.heroAnchor.centerXPct) / 100;
  ...
  if (y == null && sz && sz.heroBands && Array.isArray(sz.heroBands.profile)) {
    ...
    const best = pool.reduce((a, b) => (Math.abs(b.topPct - pref) < Math.abs(a.topPct - pref) ? b : a));
...
// for quiet briefs — "keep it quiet / every word must be readable" wants no setpiece at all).
const HEROLESS = !theme.hero;
if (HEROLESS) theme.hero = {};
const hero = theme.hero.match ? findPhrase(theme.hero.match) : null;
if (!hero && !HEROLESS)
  throw new Error('[make-theme] theme.json requires hero:{match} (omit hero entirely for a quiet pure-body run)');
...
const heroOut = hero ? Math.min(DUR - 0.06, theme.hero.out ?? (MAXHOLD > 0 ? heroIn + MAXHOLD : DUR - 0.1)) : -999;
...
if (!heroInline && !HEROLESS) {
  if (dna.hero.setpiece === 'detonation') {
    ...
    const CPM = JSON.parse(fs.readFileSync(path.join(SKILL, 'assets/brand/cyberpunk-widths.json'), 'utf8'));
  ...
  if (HG.halfW) HG.x = 2 * HG.halfW + 24 > W ? W / 2 : Math.max(HG.halfW + 12, Math.min(W - HG.halfW - 12, HG.x));
...
function fontCssFor(pageParts) {
  ...
  for (const fam of fams) {
    ...
    const famRe = new RegExp(`font-family\\s*:\\s*['"]` + fam.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + `['"]`);
    const hits = FONT_FACES.filter((b) => famRe.test(b));
    if (!hits.length)
      console.error(`[make-theme] WARN font "${fam}" not in bundled fonts.css -> renderer fallback risk`);
    out.push(...hits);
  }
  return out.join('\n');
}
...
function paradigmPanel() {
  const b = dna.body;
  const lockT = heroIn + 0.3 + (dna.hero.params.lockStagger || 0.045) * (heroText.length - 1) + 0.083;
  const lineData = LINES.map((L) => ({
    id: 'ln' + L.id,
    words: L.words.map((w) => [
      w.display,
...
function paradigmPoem() {
  ...
  const stanzaData = stanzas.map((s, si) =>
    s.map((L, li) => ({
      ...
      words: L.words.map((w) => [w.display, +w.start.toFixed(3), w.isHero && heroInline ? 'big' : w.minor ? 'em' : '']),
    })),
  );
...
function frontFx() {
  ...
  if (fx.confetti) {
    ...
    const CC = dna.palette.conf || [dna.palette.cream || '#f6ecd4', dna.palette.accent, dna.palette.body];
    css += `
...
function paradigmLastpage() {
  ...
  for (let i = 0; i < (b.fieldCount || 9); i++) {
    let x = 80 + prnd() * (W - 160),
      y = 60 + prnd() * (H * 0.55);
    if (Math.abs(x - subjX) < 170) x = x < subjX ? subjX - 180 - prnd() * 120 : subjX + 180 + prnd() * 120;
    inst.push({
...
function paradigmLedboard() {
  ...
  const pins = LINES.map((L, i) =>
    i === 0 ? Math.max(0.02, +(L.words[0].start - 0.22).toFixed(3)) : +(L.words[0].start - 0.22).toFixed(3),
  );
  ...
  for (let i = 0; i + 1 < starts.length; i++)
    if (starts[i + 1] - starts[i] > 1.2)
      for (let t = starts[i] + 0.43; t < starts[i + 1] - 0.55; t += 0.6) shimmers.push(+t.toFixed(3));
  ...
  const statBlinks = [swapT - 2.55, swapT - 1.75, swapT - 1.15].filter((t) => t > 0.25).map((t) => +t.toFixed(3));
...
` +
    lineData.map((L) => `      <div class="vln" id="${L.id}"></div><div class="vlnE" id="${L.eid}"></div>`).join('\n') +
    `
      <div id="drift"></div>
  ...
  const gags = lineData.filter((L) => L.gag).map((L) => [+(L.out - 0.01).toFixed(3), +(L.out + 0.35).toFixed(3)]);
...
function paradigmCarbonstrip() {
  ...
  const rgb = (h) => [h.slice(1, 3), h.slice(3, 5), h.slice(5, 7)].map((x) => parseInt(x, 16)).join(',');
...
function paradigmLaserrail() {
  ...
  const lineData = LINES.map((L, i) => {
    ...
    const pos = words.map((w, k) => {
      ...
      return [Math.round(cx), Math.round(baseline - (pxs[k] * (m.ascent - m.descent)) / 2), Math.round(ws[k])];
    });
    ...
    const sweepT = i + 1 < LINES.length ? +(L.out - 0.14).toFixed(3) : +Math.min(L.out - 0.14, DUR - 0.26).toFixed(3);
    return {
      id: 'lr' + L.id,
      in: +L.in.toFixed(3),
      sweepT,
...
function paradigmPlanktonrail() {
  ...
  const html =
    `      <div id="mfield"></div>
` + lineData.map((L) => `      <div class="pln" id="${L.id}" style="top:${L.top}px"></div>`).join('\n');
  const js = `
...
function paradigmSheenrail() {
  ...
  const html =
    `      <div id="skwrap">
        <div id="band"></div>
` +
    lineData.map((L) => `        <div class="skl" id="${L.id}" style="top:${L.top}px"></div>`).join('\n') +
    `
      </div>`;
  const js = `
...
function paradigmScoperail() {
  ...
  const lineData = LINES.map((L, i) => {
    ...
    const next = LINES[i + 1];
    const out = next ? +Math.min(next.in - 0.05, lastSt + 0.67).toFixed(3) : +Math.min(NZ + 0.33, DUR - 0.2).toFixed(3);
    return {
      id: 'sc' + L.id,
      in: +L.in.toFixed(3),
      out,
...
function paradigmPaperrail() {
  ...
  const lineData = LINES.map((L, li) => {
    ...
    const xo = +(li === LINES.length - 1 ? Math.min(L.out - 0.04, DUR - 0.2 - (n - 1) * 0.03) : L.out - 0.04).toFixed(
      3,
    );
...
function paradigmChalkrail() {
  ...
  for (let pi = R; pi < LINES.length; pi += R) {
    ...
    const t = +Math.max(prevEnd + 0.05, Math.min(prevEnd + (b.eraseDelay ?? 0.23), nextIn - 0.9)).toFixed(3);
...
function paradigmMarkerrail() {
  ...
  // apex tag writes.
  const b = dna.body;
  const EMS = [dna.palette.em || dna.palette.accent, dna.palette.em2 || dna.palette.em || dna.palette.accent];
  let emIdx = 0;
  const lineData = LINES.map((L, li) => ({
    id: 'mr' + L.id,
    in: +L.in.toFixed(3),
...
function paradigmInkrail() {
  ...
  const lineData = LINES.map((L, li) => {
    ...
    if (li + 2 < n) {
      ...
      outT = +Math.max(L.words[L.words.length - 1].start + 0.25, LINES[li + 2].words[0].start - outDur - 0.05).toFixed(
        3,
      );
    }
    return {
  ...
  const html = lineData.map((L) => `      <div class="irow" id="${L.id}" style="top:${L.y}px"></div>`).join('\n');
  const js = `
...
function setpieceCoverword() {
  ...
  const CPM = JSON.parse(fs.readFileSync(path.join(SKILL, 'assets/brand/cyberpunk-widths.json'), 'utf8'));
  const fontB64 = fs.readFileSync(path.join(SKILL, 'assets/brand/CyberpunkReplica.ttf')).toString('base64');
  const DISP = HG.coverDisp || heroText[0].toUpperCase() + heroText.slice(1).toLowerCase();
  const hpx = HG.fontPx;
  const em = HG.coverEm || [...DISP].reduce((a, c) => a + (CPM.widths[c] || 0.7), 0) + 0.01 * (DISP.length - 1);
  ...
  const clipDefs = bnd
    ...
    .join('');
  const layer = (fill) => `<rect x="0" y="0" width="${BW}" height="${BH}" fill="${fill}" mask="url(#cwm)"/>`;
  const EX = theme.hero.exitAt ?? Math.min(heroOut - 0.2, I + (p.hold ?? 2.6));
  const css = `
...
function setpieceBossintro() {
  ...
  const endPulse = LASTWORD.start + 0.384 < DUR - 0.02 && LASTWORD.start > EX + 0.4 ? LASTWORD.start : null;
...
function setpieceBoltstrike() {
  ...
  const kissData = LINES.flatMap((L) => L.words.map((w) => [+w.start.toFixed(3), w.minor ? 1 : 0])).filter(
    ([t]) => t < DUR - 0.1,
  );
  const css = `
...
function setpieceSilkribbon() {
  ...
  const D = execFileSync(
    'python3',
    [gen, fontPath, heroDisplay.toLowerCase(), String(tw), '200', String(Math.round((svgW - tw) / 2))],
    { encoding: 'utf8' },
  ).trim();
...
function setpieceScopetrace() {
  ...
  const svgW = tw + 140,
    ...
    CY = BY - 28;
  const D = execFileSync('python3', [gen, fontPath, traceText, String(tw), String(BY), String(70)], {
    encoding: 'utf8',
  }).trim();
...
function setpieceCenterfold() {
  ...
  const XO = +Math.max(I + (segs - 1) * stag + 0.45, Math.min(heroOut - 0.33, DUR - 0.39)).toFixed(3);
...
function setpieceChalkwrite() {
  ...
  let D = execFileSync('python3', [gen, fontPath, heroDisplay.toLowerCase(), String(tw), '450', String(x0)], {
    encoding: 'utf8',
  }).trim();
...
function setpieceBrushwrite() {
  ...
  const html = `      <div id="bwWash"></div>
      ...
      <div id="bwSeal"><span>${esc(p.sealGlyph || 'W')}</span></div>`;
  const js = `
...
function setpieceInkbloom() {
  ...
  const XOUT = +Math.max(
    C + 0.55,
    Math.min(D + (p.maxHold ?? 1.75), nextL ? nextL.words[0].start - (p.clearGap ?? 0.24) : DUR - 0.6, DUR - 0.6),
  ).toFixed(3);
...
function setpieceRansomnote() {
  ...
  let ripT = Math.min(theme.hero.exitAt ?? LASTWORD.start - (p.ripLead ?? 0.19), DUR - 0.22 - RSTAG * (N - 1));
