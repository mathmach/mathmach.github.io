  ...
  try {
    ...
    page.on('request', (req) => {
      ...
      if (req.resourceType() === 'script' && /gsap/i.test(u) && /^https?:/i.test(u)) {
        if (gsapSource) req.respond({ status: 200, contentType: 'application/javascript', body: gsapSource });
...
async function main() {
  ...
  let fps = 24;
  try {
    const f = parseFloat(String(fs.readFileSync(path.join(project, 'matte.fps'), 'utf8')).replace(/[^\d.]/g, ''));
    if (f > 0) fps = f;
  } catch {}
  ...
  let globalFg = false;
  try {
    globalFg = JSON.parse(fs.readFileSync(path.join(project, 'plan.json'), 'utf8')).caption_layer === 'fg';
  } catch {}
