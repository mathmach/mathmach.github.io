...
async function sceneOptics(project, bgPath, fgPath, zones, subjectBox, W, H) {
  ...
  try {
    if (bigZone && bigZone.w >= 64 && bigZone.h >= 64) bgSharp = await regionSharpness(bgPath, bigZone, W, H);
  } catch {}
  try {
    if (subjectBox && subjectBox.w >= 64) subjSharp = await regionSharpness(bgPath, subjectBox, W, H);
  } catch {}
