...
// ─── 1. tokens.css ────────────────────────────────────────────────
const rootMatch = html.match(new RegExp(`${PRE_OPEN}<!--\\s*ROOT-START\\s*-->([\\s\\S]*?)<!--\\s*ROOT-END\\s*-->`));
if (!rootMatch) {
  console.error(
...
const voiceMatch = html.match(new RegExp(`${PRE_OPEN}<!--\\s*VOICE-START\\s*-->([\\s\\S]*?)<!--\\s*VOICE-END\\s*-->`));
if (!voiceMatch) {
  console.error(
...
const hintsMatch = html.match(new RegExp(`${PRE_OPEN}<!--\\s*HINTS-START\\s*-->([\\s\\S]*?)<!--\\s*HINTS-END\\s*-->`));
let hintsFile = null;
if (hintsMatch) {
...
while ((cm = compRe.exec(html)) !== null) {
  ...
  if (metaMatch) {
    try {
      ...
      console.error(`! emit-chunks: component '${id}' has COMPONENT-META but JSON is invalid (${e.message}); ignoring`);
    }
