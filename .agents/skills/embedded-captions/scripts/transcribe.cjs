...
function audibleEnd(audio) {
  try {
    const r = cp.spawnSync(
      'ffmpeg',
      ['-hide_banner', '-nostats', '-i', audio, '-af', 'silencedetect=noise=-35dB:d=0.6', '-f', 'null', '-'],
      { encoding: 'utf8' },
    );
    const out = (r.stderr || '') + (r.stdout || '');
...
function main() {
  ...
  if (!fs.existsSync(audio))
    cp.execFileSync('ffmpeg', ['-y', '-i', src, '-vn', '-acodec', 'libmp3lame', '-q:a', '2', audio], {
      stdio: 'ignore',
    });
  ...
  if (wantWx) {
    try {
      ...
      if ((r.status || 0) !== 0)
        throw new Error((r.stderr || 'whisperx failed').split('\n').slice(-4).join(' ').slice(0, 300));
