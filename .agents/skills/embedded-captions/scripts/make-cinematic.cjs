...
function main() {
  ...
  if (missing.length)
    die(`words not in transcript (in order): ${missing.join(' ')} — lines must match the transcript verbatim`);
  ...
  try {
    ...
    for (const f of ['source.mp4'].concat(fs.readdirSync(project).filter((x) => /\.(mp4|mov|webm)$/i.test(x)))) {
      ...
      const d = parseFloat(
        cp.execFileSync(
          'ffprobe',
          ['-v', 'error', '-show_entries', 'format=duration', '-of', 'default=nokey=1:noprint_wrappers=1', fp],
          { encoding: 'utf8' },
        ),
      );
      if (d > 0) {
  ...
  const planeBudgetPx = (pk) => {
    const css =
      (C.planes && C.planes[pk] && (typeof C.planes[pk] === 'string' ? C.planes[pk] : C.planes[pk].css)) || '';
  ...
  blocks.forEach((b, bi) => {
    ...
    if (hs.length > 1)
      die(`block ${bi} has ${hs.length} hero lines — at most ONE hero per block (scarcity = per beat)`);
  ...
  const lockupTemplate =
    C.planes && C.planes.lockup ? (typeof C.planes.lockup === 'string' ? C.planes.lockup : C.planes.lockup.css) : null;
  ...
  if (sz && sz.zones) {
    ...
    for (const [pk, pv] of Object.entries(C.planes || {})) {
      ...
      if (best.meanLuma > 150 || pkL > 175)
        planeComp[pk] = ' mix-blend-mode: normal; text-shadow: 0 1px 6px rgba(0,0,0,.65), 0 2px 16px rgba(0,0,0,.4);';
  ...
  for (const [pk, ls] of Object.entries(byPlane)) {
    ...
    pages.forEach((pg, pi) => {
      ...
      pg.forEach((l, li) => {
        ...
        if (li === 0 && pi > 0)
          l.in = +Math.min(l.first, Math.max(l.first - LEAD, pages[pi - 1][0].out - 0.02)).toFixed(3);
      });
    });
  }
  let gid = 0;
          ...
          : `position:absolute;left:0;right:0;top:${l.slotPx}px; ` +
            (() => {
              const pc =
                (C.planes &&
                  C.planes[l.plane] &&
                  (typeof C.planes[l.plane] === 'string' ? C.planes[l.plane] : C.planes[l.plane].css)) ||
                '';
  ...
  for (const hRef of heroRefs) {
    ...
    groups.push({
      ...
      css:
        ...
        ensureFontSize(ln.css, 'font-size: calc(0.24 * var(--h)); font-weight: 900; white-space: nowrap;'), // case/tracking come from the DNA's hero treatment
      words: ln._w.map((w, i) => ({
    ...
    if (!heroFeasible)
      console.log('[make-cinematic] no hero band ≤62% predicted occlusion → hero rendered in FRONT (last resort)');
  }
  ...
  // wrapped tail. Measure every page's REAL text heights in Chromium and re-stack.
  {
    ...
    for (const l of flat) {
      ...
      (pagesByKey[key] = pagesByKey[key] || { t: +(l.out - 0.1).toFixed(2), lines: [] }).lines.push(l);
    }
    ...
    if (times.length) {
      cp2.spawnSync('node', [path.join(__dirname, 'measure-layout.cjs'), project, ...times.map(String)], {
        stdio: 'ignore',
        timeout: 120000,
      });
      let lay = null;
    ...
    }
  {
    ...
    if (nwChanged) {
      fs.writeFileSync(path.join(project, 'plan.json'), JSON.stringify(plan, null, 2));
      require('child_process').spawnSync('node', [path.join(__dirname, 'make-composition.cjs'), project], {
        stdio: 'ignore',
      });
    }
  }
  ...
  for (const hRef of heroRefs) {
    ...
    const measure = () => {
      ...
      cp2.spawnSync('node', [path.join(__dirname, 'measure-layout.cjs'), project, String(midT), String(endT)], {
        stdio: 'ignore',
        timeout: 90000,
      });
      try {
    ...
    for (let pass = 0; pass < 5; pass++) {
      ...
      const others = caps
        .filter(
          (c) =>
            c.id !== heroG.id && !mateIds.has(c.id) && !/-glow$/.test(c.id || '') && c.cap_bbox && c.opacity > 0.05,
        )
        ...
        if (hc.cap_bbox.w > maxW) {
          ...
          if (nf > f0 * 1.05 || fit > dnaSizeCap * 1.02) {
            ...
            if (fit > dnaSizeCap && wAtCap < maxW * 0.85 && capsy) {
              ...
              if (tr >= 0.04) {
                heroG.css =
                  heroG.css.replace(/letter-spacing\s*:[^;]+;?\s*/g, '').replace(/text-indent\s*:[^;]+;?\s*/g, '') +
  ...
  // column overflow gets a loud note (centered spill is legal on a clear side only).
  {
    ...
    for (const mg of minors) {
      ...
      let mc = null;
      try {
        mc = (JSON.parse(fs.readFileSync(path.join(project, '_layout.json'), 'utf8')).samples[0].caps || []).find(
          (c) => c.id === mg.id,
        );
      } catch {}
