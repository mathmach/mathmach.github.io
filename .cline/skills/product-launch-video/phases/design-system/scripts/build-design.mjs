...
const captureDir = cliCaptureDir ? path.resolve(cliCaptureDir) : path.resolve(outDir, '..', 'capture');
...
function _brandClassify(stats, buttons) {
  ...
  return stats
    .map((v) => {
      ...
      let score =
        v.bgCount * 3 + v.interactiveBg * 18 + v.areaBg * 2 + sat * 1.3 + Math.log10(Math.max(1, v.count)) * 4;
...
function _tripletFromChrom(chrom, primaryOverride) {
  ...
  const primary = primaryOverride && /^#[0-9A-F]{6}$/.test(primaryOverride) ? primaryOverride : chrom[0].hex;
  const secondary = (
  ...
  const accent = (chrom.filter((x) => !taken.has(x.hex)).sort((a, b) => b.sat - a.sat)[0] || { hex: secondary }).hex;
...
function deriveBrandColors() {
  ...
  const taken = new Set([primary, secondary]);
  const accentCands = palette.filter((c) => !taken.has(c) && !_isGrayish(c)).sort((a, b) => _sat(b) - _sat(a));
...
function deriveVoice() {
  ...
  const avgWords = headings.length
    ? headings.reduce((s, h) => s + (h.text || '').split(/\s+/).filter(Boolean).length, 0) / headings.length
    : 0;
...
const tokens = {
  ...
  primitive: {
    ...
    border: Object.fromEntries(_borderStrings.map((s, i) => [`b${i}`, { $value: s, $type: 'border' }])),
  },
};

const motion = {
  duration: {},
  easing: Object.fromEntries(Object.entries(_easings).map(([k, v]) => [k, { $value: v, $type: 'easing' }])),
};
...
const dna = {
  materialLanguage: {
    ...
    metrics: {
      ...
      gradientCount: Object.values(hfTokens.cssVariables || {}).filter((v) => /gradient/i.test(String(v))).length,
    },
  },
...
function brandFromBrandHtml(html) {
  ...
  const get = (cls) => {
    ...
    const re = new RegExp(`brand-color-${cls}[\\s\\S]*?big-swatch-hex"[^>]*>\\s*(#[0-9a-fA-F]{3,6})`);
...
let primaryHex = brandFromHtml?.primary || valueOf(primColors.brand?.primary) || allColors[0] || '#000000';
let secondaryHex =
...
let tertiaryHex =
  ...
  (() => {
    const taken = new Set([primaryHex, secondaryHex, accentHex, canvasHex, inkHex].map((c) => c.toLowerCase()));
    const candidates = allColors
      .filter((c) => !taken.has(c.toLowerCase()))
      .map((c) => ({ c, s: saturation(c) }))
      .filter((o) => o.s > 0.3)
    ...
    if (req.kind === 'block_installed') {
      ...
      if (!fileOk || !libOk) {
        missing.push({
          kind: req.kind,
          block: req.block,
          missing_files: [!fileOk ? req.verify_file : null, !libOk ? req.verify_lib : null].filter(Boolean),
...
function detectSiteFeatures() {
  ...
  return {
    ...
    condensed_display: /Anton|Archivo Black|Bebas|Oswald|Space Grotesk/i.test(fontFamilies.join(' ')),
    serif_display: /Serif|Fraunces|Spectral|Newsreader|Playfair|Garamond|Times/i.test(fontFamilies.join(' ')),
...
function pickPreset() {
  ...
  if (cliStyle) {
    ...
    if (!forced) {
      console.error(`✗ unknown --style '${cliStyle}'. available: ${presets.map((p) => p.name).join(', ')}`);
      process.exit(1);
    }
  ...
  for (const manifestPath of manifestCands) {
    ...
    try {
      ...
      for (const rec of files) {
        ...
        const familyDisplay =
          unwrapped.length <= 4 ? unwrapped.toUpperCase() : unwrapped.replace(/\b([a-z])/g, (_, c) => c.toUpperCase());
...
const script = rawScript || presetFontFallback.script ? resolveFont(rawScript, 'script', presetFontFallback) : null;
...
function renderCaptions() {
  ...
  const srcdoc = skin.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
  return `
...
function renderVoice() {
  ...
  const dnaLines = [
    ...
    sampleHeadings.length && '- Sample headings:\n' + sampleHeadings.map((s) => `  - ${s}`).join('\n'),
  ].filter(Boolean);
...
function placeholderFor(key) {
  ...
  const map = {
    ...
    HEADLINE_WITH_EM: 'borders are <em>structural</em>. shadows are <em>weight</em>. tilt is <em>intent</em>.',
...
function summariseCapabilityGated() {
  if (!scores) return [];
  return scores.filter((s) => s.capabilities_missing && s.capabilities_missing.length > 0).map(summariseCandidate);
}
...
console.log(`  brand:    ${brandReport.primary} primary (${brandReport.source}, confidence=${brandReport.confidence})`);
