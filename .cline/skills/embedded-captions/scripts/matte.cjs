...
function extractFrames(src, dst, fps, extra = []) {
  ...
  cp.execFileSync('ffmpeg', ['-y', '-i', src, '-vf', `fps=${fps}`, ...extra, path.join(dst, 'f_%04d.png')], {
    stdio: 'ignore',
  });
  return true;
}
...
async function main() {
  ...
  if (extractFrames(src, framesBg, fps)) console.log(`[matte] source fps=${fps} → frames_bg extracted`);
  ...
  if (isVfr(src)) {
    ...
    if (!fs.existsSync(cfr)) {
      console.log(`[matte] VFR source detected (nominal != actual fps) → normalizing to ${fps}fps CFR for matting`);
      cp.execFileSync(
        'ffmpeg',
        [
          '-y',
  ...
  const cached = fs.existsSync(
    path.join(os.homedir(), '.cache', 'hyperframes', 'background-removal', 'models', 'u2net_human_seg.onnx'),
  );
  console.log(
  ...
  if (got > want && want > 0) {
    ...
    const keep = [];
    for (let j = 1; j <= want; j++) keep.push(Math.min(got, Math.max(1, Math.round((j - 0.5) * (got / want)))));
